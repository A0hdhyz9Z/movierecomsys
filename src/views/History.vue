  ...
  <div class="tbc">
    ...
    <el-dialog title="预测" v-model="dialogFormVisible">
      <el-form :model="predictForm">
        <el-form-item :label="'任务编号：'+(predictForm.id)" :label-width="formLabelWidth">
          <!-- <p>{{predictForm.id}}</p> -->
        </el-form-item>
      ...
      selectAll().then(res => {
        if (res.code == 200) {  //上传成功
          ...
          for (var key in obj) { //遍历数组中的对象
            ...
            if (obj[key].orderStatus == 2) {
              data.orderStatus = "失败"
            }
            if (obj[key].orderStatus == 3) {
              data.orderStatus = "失败"
            }
            dataArr.push(data)
          }
...
.per_btn:hover:before,
.per_btn:focus:before {
  transition: 0.5s all ease;
  left: 0;
  right: 0;
  opacity: 1;
